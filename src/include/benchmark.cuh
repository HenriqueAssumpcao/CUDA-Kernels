...
void benchmark_kernel(const size_t nruns, const size_t M, const size_t N, const size_t K, const int seed, const float TOL,
                      RunKernelFunc run_kernel, const std::string& kernel_name = "")
{
    ...
    srand(seed);
    rand_matrix(A); rand_matrix(B);
    A.to(CUDA_ID); B.to(CUDA_ID); C.to(CUDA_ID);
    ...
    for(size_t runIdx = 0; runIdx < nruns; ++runIdx) {
        ...
        if (err != cudaSuccess) {
            std::cerr << "CUDA error in " << kernel_name << ": " << cudaGetErrorString(err) << std::endl;
            break;
        }
    }
    ...
    std::cout << kernel_name << " Runtime (ms): " << mean << " ± " << std_dev 
              << " [min: " << runtimes[0] << ", max: " << runtimes.back() << "]" << std::endl;
    
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
    A.memfree(); B.memfree(); C.memfree();
}